...
const Shows = (props) => {
  ...
  useEffect(() => {
    fetchFashionShowsByAlpha(
      route?.params?.fashionweekId
        ? route?.params?.fashionweekId
        : route?.params?.cityEvent?.fashionweek_id
    );
  }, [changeView]);

  const windowHeight = Dimensions.get("window").height;
  const showLetters = fashionShowsByAlpha.length
    ? Object.keys(fashionShowsByAlpha[0].indexes).map((key) => (
        <TouchableOpacity
          onPress={() => {
  ...
  const renderShowsByAlphabet = fashionShowsByAlpha.length ? (
    Object.keys(fashionShowsByAlpha[0].indexes).map((key, index) => (
      <BrowseByAlphabet
  ...
  const renderShowsByDates =
    fashionShows && Object.keys(fashionShows[0].days).length ? (
      Object.keys(fashionShows[0].days).map((key, index) => (
        <BrowseByDate
          navigation={navigation}
          show={fashionShows[0].days[key]}
          key={index}
          date={key}
        />
      ))
              ...
              <View style={styles.headingMainContainer}>
                <View style={styles.showsHeadingContainer}>
                  ...
                  <Text style={styles.heading}>
                    {fashionShows &&
                      fashionShows[0]?.title.replace(/&amp;\s*\/?/gm, "& ")}
                  </Text>
                </View>
                <TouchableOpacity onPress={() => setChangeView(!changeView)}>
                  <Text style={styles.browseBtn}>
                    {changeView ? "Browse by Date" : "Browse A-Z"}
                  </Text>
                </TouchableOpacity>
              </View>
              {changeView ? renderShowsByAlphabet : renderShowsByDates}
            </View>
          </ScrollView>
        </View>
      )}
