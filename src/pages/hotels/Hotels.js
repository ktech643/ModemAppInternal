...
const Hotels = (props) => {
  ...
  const showLetters = citieshotel?.length
    ? Object.keys(citieshotel[0].indexes).map((key) => (
        <TouchableOpacity
          onPress={() => {
  ...
  const renderHotels =
    citieshotel && Object.keys(citieshotel[0]?.indexes).length ? (
      Object.keys(citieshotel[0].indexes).map((index, key) => {
        ...
        return (
          ...
          >
            {citieshotel[0].indexes[index].length ? (
              <Text style={styles.letter}>{index}</Text>
            ) : null}
            {citieshotel[0].indexes[index].map((data, key) => (
              <HotelByAlphabet key={key} citieshotel={data} />
            ))}
          </View>
        );
      })
  ...
  const renderCities = neighbourhoods.length ? (
    neighbourhoods.map((neighbourhood, i) => (
      <MultiLabelStoreByCity city={neighbourhood} key={i} />
    ))
  ) : (
    <Text style={styles.noEvents}>There are no neighbourhoods.</Text>
  );
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.rootContainer}>
            ...
            {neighbourhoods.length ? (
              <ScrollView
                style={styles.headingMainContainer}
                horizontal={true}
              ...
              {!selectedIndex ? (
                <>
                  <Text style={styles.gernalHeading}>
                    {citieshotel && citieshotel[0]?.title}
                  </Text>
                  {renderHotels}
                </>
              ) : (
