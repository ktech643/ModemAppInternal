...
const CitiesEvents = (props) => {
  ...
  const [filteredEvents, setFilteredEvents] = useState(
    (brandEventsData?.length && brandEventsData) || []
  );
  ...
  const showLetters =
    filteredEvents?.length && data?.length
  ...
  useEffect(() => {
    ...
    if (!selectedDate)
      return setFilteredEvents(brandEventsData?.length && brandEventsData);

    brandEventsData?.length &&
      Object.keys(brandEventsData[0].events).map((key) => {
  ...
  const renderShowrooms = filteredEvents?.length ? (
                ...
                <>
                  <View style={styles.bottomBorder}>
                    <Text style={styles.gernalHeading}>
                      {brandEventsData && brandEventsData[0]?.title}
                    </Text>
                  </View>
                  {renderShowrooms}
                </>
