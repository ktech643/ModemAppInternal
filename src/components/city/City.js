...
const City = (props) => {
  ...
  const handleShows = (link) => {
    ...
    if (
      [
        "AgendaTradeShows",
        "AgendaMultiLabelShowrooms",
        "AgendaBrandsShowrooms",
      ].includes(link)
    )
      navigation.navigate("Agenda", {
        screen: link,
        params: { cityEvent: cityEvent && cityEvent },
      });
    else navigation.navigate(link, { cityEvent: cityEvent });
  };
  const renderCityEventTypes = cityEvent.event_type.map((type, key) => (
    <TouchableOpacity
      key={key}
      onPress={() => handleShows(showsList[type]?.link)}
    >
      <Animatable.View
      ...
      >
        ...
        <Text style={styles.listText}>
          {showsList[type] ? showsList[type]?.text : type}
        </Text>
      </Animatable.View>
    </TouchableOpacity>
  ));
  ...
  const showCityDetails = (
    <View style={styles.showCityDetailsContainer}>{renderCityEventTypes}</View>
  );
  ...
  return (
    <View style={styles.cityContainer}>
      <TouchableOpacity onPress={handleView} style={styles.cityDetailsBtn}>
        ...
        {cityEvent.fashionweek_id === openedCity && (
          <Text style={styles.date}>
            {cityEvent.dates_collection.replace(/&amp;\s*\/?/gm, "& ")}
          </Text>
        )}
      </TouchableOpacity>
