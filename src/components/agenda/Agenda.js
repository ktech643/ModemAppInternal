...
const Agenda = (props) => {
  ...
  const renderAllAgendas = compaignList.map((compaign, key) => (
    <TouchableOpacity
      key={key}
      onPress={() => navigation.navigate(compaign.link, { other: true })}
    >
      <Animatable.View
  ...
  const showCityDetails = (
    <View style={styles.showCityDetailsContainer}>{renderAllAgendas}</View>
  );
